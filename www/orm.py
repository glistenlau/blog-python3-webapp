...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def save(self):
        ...
        if rows != 1:
            logging('failed to insert record: affected rows: %s' % rows)
    ...
    def remove(self):
        ...
        if rows != 1:
            logging('failed to remove by primary key: affected rows: %s' % rows)
